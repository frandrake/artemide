-- v1.2: extend audit_log.action CHECK to accept the new actions
-- (approve, ack, denied). SQLite cannot ALTER a CHECK, so rebuild — audit_log
-- is a leaf table (referenced by no FK), safe within the runner's transaction.
